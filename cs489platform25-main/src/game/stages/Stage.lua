...
local Explosion = require "src.game.Explosions"
...
function Stage:init(rows, cols, ts)
    ...
    self.particles = {} -- stores explosions/particles for when a mob dies
end
...
function Stage:update(dt)
    ...
    for k=#self.particles, 1, -1 do
        -- check if the particle still being displayed
        if self.particles[k]:isActive() then
            self.particles[k]:update(dt) -- update with the time since last frame
        else
            table.remove(self.particles, k) -- not displayed, remove it from the particles table
        end
    end
end
...
function Stage:draw()
    ...
    for k=1, #self.particles do
        self.particles[k]:draw() -- draws the particle effects when appropriate
    end
end
...
function Stage:createExplosion(x, y)
    local exp = Explosion() 
    exp:trigger(x, y) -- trigger the explosion at the specified coords
    table.insert(self.particles, exp) -- add the explosion to table of particles
end
